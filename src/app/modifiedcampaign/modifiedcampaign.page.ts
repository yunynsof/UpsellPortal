...
@Component({
  selector: 'app-modifiedcampaign',
  templateUrl: './modifiedcampaign.page.html',
  styleUrls: ['./modifiedcampaign.page.scss'],
})
export class ModifiedcampaignPage implements OnInit {
  ...
  ngOnInit() {
    ...
    this.campaignId = this.navParams.data.paramID;
    this.getCampaign();
  ...
  next() {
    ...
    if (this.slideFirstValidator()) {
      if (this.validateDateEnd()) {
        if (this.art.event == null && this.art.prize == null) {
          this.art.event = this.eventSelect;
          this.art.prize = this.prizeSelect;
        }
        this.getSummary();
    ...
    this.authService.getCampaign( this.alertService)
      .then(data => {
        ...
        for (let i = 0; i < this.campaign.items.length; i++) {
          if (this.campaign.items[i].id == this.campaignId) {
            this.campaign.items[i].start = this.campaign.items[i].start;
            this.campaign.items[i].end = this.campaign.items[i].end;
            this.itemsCampaign = this.campaign.items[i];
          }
        }
        this.obtainDataCampaign(this.itemsCampaign);
      });
  ...
  createCampaignForm(form) {
    ...
    this.authService.modifiedCampaignForm(this.campaignId, this.instance).subscribe((res) => {
      this.alertService.dismiss();
      this.alertService.presentToast("Se ha guardado la modificacion correspondiente");
    }, error => {
  ...
  obtainDataCampaign(data) {

    this.art.name = data.name;
    this.groupSelect = data.groupId;

    this.mydate1 = data.start;
    this.mydate2 = data.end;
    this.art.typeBase = data.databaseType;
    this.art.excludeNotifUpsellFramework = data.excludeNotifUpsellFramework;
    this.idExaSelect = data.exTemplateId;

    this.eventSelect = data.events[0].idEventTyT;
    this.amountList = data.events[0].startEventAmount;
    this.art.amount = this.amountList;
    this.prizeSelect = data.events[0].prizeCpeProductId;
    this.listNotification = data.notifications;

    setTimeout(() => {
      this.art.group = this.groupSelect;
      this.art.idExa = this.idExaSelect;
      this.alertService.dismiss();
    }, 1000);
    this.obtainNotifcampaign(this.listNotification)
  }

  obtainNotifcampaign(listNotification) {

    for (let i = 0; i < listNotification.length; i++) {
      if (listNotification[i].notificationType == "START") {

        this.art.hrStartCamp = this.getHourNotification(listNotification[i].schedules[0].startHour, listNotification[i].schedules[0].startMinutes);
        this.dayMessage.push({
          id: listNotification[i].id,
          exacaster: this.art.idExa,
          typeMessage: listNotification[i].notificationType,
          message: listNotification[i].body,
          description: this.obtainDescriptionNotif(),
        ...
        if (listNotification[i].notificationType == "END") {
          ...
          if (listNotification[i].notificationType == "REMINDER") {

            for (let a = 0; a < listNotification[i].schedules.length; a++) {
              if (listNotification[i].schedules[a].weekday == "MON") {
                this.art.lun = true;
                this.art.hrLun = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
              if (listNotification[i].schedules[a].weekday == "TUE") {
                this.art.mar = true;
                this.art.hrMar = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
              if (listNotification[i].schedules[a].weekday == "WED") {
                this.art.mie = true;
                this.art.hrMie = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
              if (listNotification[i].schedules[a].weekday == "THU") {
                this.art.jue = true;
                this.art.hrJue = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
              if (listNotification[i].schedules[a].weekday == "FRI") {
                this.art.vie = true;
                this.art.hrVie = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
              if (listNotification[i].schedules[a].weekday == "SAT") {
                this.art.sab = true;
                this.art.hrSab = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
              if (listNotification[i].schedules[a].weekday == "SUN") {
                this.art.dom = true;
                this.art.hrDom = this.getHourNotification(listNotification[i].schedules[a].startHour, listNotification[i].schedules[a].startMinutes);
              }
            }
            this.dayMessage.push({
              id: listNotification[i].id,
  ...
  getHourNotification(hour, minutes) {
  ...
  obtainDescriptionNotif() {

    let description = "";
    if (this.art.lun) {
      description = "Lun=" + this.art.hrLun;
    }
    if (this.art.mar) {
      description += " Mar=" + this.art.hrMar;
    }
    if (this.art.mie) {
      description += " Mie=" + this.art.hrMie;
    }
    if (this.art.jue) {
      description += " Jue=" + this.art.hrJue;
    }
    if (this.art.vie) {
      description += " Vie=" + this.art.hrVie;
    }
    if (this.art.sab) {
      description += " Sab=" + this.art.hrSab;
    }
    if (this.art.dom) {
      description += " Dom=" + this.art.hrDom;
    }
    if (this.art.hrStartCamp) {
      description += " Hora = " + this.art.hrStartCamp;
    }
    return description;
  }
  ...
  selectMessage(art) {
    ...
    if (art.typeMessage == "REMINDER") {
      ...
      if (art.hrStartCamp.length <= 8) {
        this.art.hrStartCamp = new Date(Date.parse("2020/01/01 " + art.hrStartCamp))
      } else
        if (art.hrStartCamp.length == undefined) {
          this.art.hrStartCamp = art.hrStartCamp;
        }
    }
  }
  ...
  saveEvent() {
    ...
    if (this.eventsManager.length > 0) {
      this.eventsManager = [];
    }
  ...
  getDateFormat(data) {
    ...
    return date.getDate() + "/" + month + "/" + date.getFullYear() + " " + date.getHours() + ":" + (date.getMinutes() < 10 ? '0' : '') + date.getMinutes() + ":" + date.getSeconds();
  }
  ...
  validateDateStart() {
    ...
    if (dateLocal.getTime() == dateForm.getTime()) {
      this.mydate1 = null;
  ...
  slideFirstValidator(): boolean {
    ...
    if (this.disablePrevBtn) {
      ...
      if (validator.isEmpty(this.validationsCreateCampaign_form.value.group.toString())) {
      ...
      if (validator.isEmpty(this.validationsCreateCampaign_form.value.idExa.toString())) {
        this.createCampaignFormValidator.idExa.empty = ' ';
      } else {
        this.createCampaignFormValidator.idExa.empty = '';
      }
      ...
      if (validator.isEmpty(this.validationsCreateCampaign_form.value.event.toString())) {
        this.createCampaignFormValidator.event.empty = ' ';
      } else {
        this.createCampaignFormValidator.event.empty = '';
      }

      if (validator.isEmpty(this.validationsCreateCampaign_form.value.prize.toString())) {
